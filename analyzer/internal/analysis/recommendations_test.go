...
func TestBuildRecommendationsManyMediumLayers(t *testing.T) {
	layers := make([]registry.ManifestLayer, 26)
	for i := range layers {
		layers[i] = registry.ManifestLayer{
			Digest: "sha256:layer",
			Size:   20 * 1024 * 1024,
...
func TestBuildRecommendationsGzipLayers(t *testing.T) {
	layers := []registry.ManifestLayer{
		{Digest: "sha256:layer", Size: 5 * 1024 * 1024, MediaType: "application/vnd.docker.image.rootfs.diff.tar.gzip"},
	}
...
func TestBuildRecommendationsPullTimeRisk(t *testing.T) {
	layers := []registry.ManifestLayer{
		{Digest: "sha256:layer", Size: 401 * 1024 * 1024},
	}
